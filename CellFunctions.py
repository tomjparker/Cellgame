...
def delete_cells(cells, cellbin):
    for cell_id in cellbin:
        if cell_id in cells:
            del cells[cell_id]


def delete_food(pantry, foodbin):
    for food_id in foodbin:
        if food_id in pantry:
            del pantry[food_id]
...
def collision(cells, player, pantry):
    ...
    for C1 in cells.values():
        for C2 in cells.values():
            if C2.cell_id != C1.cell_id and C2.cell_id in cells:
                if (abs(C1.px - cells[C2.cell_id].px) <= C1.size + cells[C2.cell_id].size) or (abs(C1.py - cells[C2.cell_id].py) <= C1.size + cells[C2.cell_id].size):
                    if C1.size > cells[C2.cell_id].size:
                        C1.size += cells[C2.cell_id].size
                        C1.energy += cells[C2.cell_id].energy
                        cellbin[C2.cell_id] = True
                    elif C1.size == cells[C2.cell_id].size:
                        break
                    else:
                        cells[C2.cell_id].size += C1.size
                        cells[C2.cell_id].energy += C1.energy
                        cellbin[C1.cell_id] = True
                        break
    ...
    delete_cells(cells, cellbin)
    delete_food(pantry, foodbin)
